...
<script>
    ...
    layui.use('table', function(){
        ...
        table.render({
            elem: '#test3'
            ,url:'${pageContext.request.contextPath}/organ/queryByLimit'
            ,cellMinWidth: 80 //全局定义常规单元格的最小宽度，layui 2.2.1 新增
            ,cols: [[
                {field:'id', width:180, title: 'ID', sort: true}
                ,{field:'name', width:180, title: '用户名',edit: 'text'}
                ,{field:'members', width:80, title: '成员数', sort: true,edit: 'text'}
        ...
        table.on('edit(test3)', function(obj){
            ...
            $.ajax({
                type: "PATCH",
                url: "${pageContext.request.contextPath}/organ/updateOrganization",
                dataType: "json",
        ...
        table.on('tool(test3)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if(layEvent === 'del'){ //删除
                layer.confirm('真的删除行么', function(index){
                    ...
                    $.ajax({
                        type: "DELETE",
                        url: "${pageContext.request.contextPath}/organ/deleteOrganization",
                        dataType: "json",
                        contentType: "application/json; charset=utf-8",
                        data: JSON.stringify({
                            "id":data.id
