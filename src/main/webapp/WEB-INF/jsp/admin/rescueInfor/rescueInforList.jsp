...
<script>
    ...
    layui.use('table', function(){
        ...
        table.render({
            elem: '#test3'
            ,url:'${pageContext.request.contextPath}/rescueInfor/queryByLimit'
            ,cellMinWidth: 80 //全局定义常规单元格的最小宽度，layui 2.2.1 新增
            ,cols: [[
                {field:'id', width:180, title: 'ID', sort: true}
                ,{field:'userId', width:180, title: '用户id',edit: 'text'}
                ,{field:'label', width:180, title: '标签', edit: 'text'}
                ,{field:'time', width:180, title: '发布时间',edit: 'text'}
                ,{field:'state', width:80, title: '状态',edit: 'text'}
                ,{field:'describe', width:180, title: '描述',edit: 'text'}
                ,{field:'locale', width:100, title: '所在地',edit: 'text'}
        ...
        table.on('edit(test3)', function(obj){
            ...
            $.ajax({
                type: "PATCH",
                url: "${pageContext.request.contextPath}/rescueInfor/updateRescueInfor",
                dataType: "json",
        ...
        table.on('tool(test3)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if(layEvent === 'del'){ //删除
                layer.confirm('真的删除行么', function(index){
                    ...
                    $.ajax({
                        type: "DELETE",
                        url: "${pageContext.request.contextPath}/rescueInfor/deleteRescueInfor",
                        dataType: "json",
                        contentType: "application/json; charset=utf-8",
                        data: JSON.stringify({
                            "id":data.id
